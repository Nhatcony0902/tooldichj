...
function safeResolve(cwd, value) {
  ...
  // SECURITY: BOTH absolute and relative paths must stay within cwd root.
  // CK upstream allowed absolute paths to bypass this check — an attacker
  // who could write `.claude/workflow-artifacts.json` with `"artifactDir": "/etc/ssl"`
  // could redirect artifact validation to arbitrary disk locations. T1K fix:
  // remove the `!path.isAbsolute(value)` guard so containment is unconditional.
...
function resolveArtifactDir({ cwd = process.cwd(), artifactDir = null, env = process.env } = {}) {
  ...
  const fromEnv = safeResolve(cwd, env.T1K_WORKFLOW_ARTIFACT_DIR);
  if (fromEnv) return { artifactDir: fromEnv, source: 'env', reasons };
  if (env.T1K_WORKFLOW_ARTIFACT_DIR) reasons.push('env artifact dir was invalid');
  ...
  if (fallback.length > 1) reasons.push('fallback is ambiguous; set T1K_WORKFLOW_ARTIFACT_DIR');
