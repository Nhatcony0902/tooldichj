...
// Runtime enforcement of the schemas' `additionalProperties: false` contract.
// The 5 *.schema.json files all declare `additionalProperties: false`, but the
// schemas are documentation-only — nothing validates against them at hook time.
// This helper rejects unknown top-level keys so a typo'd field name (e.g.
// `scoutSumary`) surfaces as an error instead of silently passing as an
// undeclared extra. Mirrors item 4 of The1Studio/theonekit-core#324.
// `allowed` is the closed set of declared keys for the artifact; only object
// values are checked (non-objects are rejected by the caller's isObject guard).
function rejectUnknownKeys(errors, value, allowed) {
  if (!isObject(value)) return;
  const known = new Set(allowed);
  for (const key of Object.keys(value)) {
    if (!known.has(key)) {
      push(errors, key, 'unknown property (schema declares additionalProperties: false)');
    }
  }
}

const CONTEXT_KEYS = ['skill', 'mode', 'task', 'scoutSummary', 'acceptanceCriteria', 'touchpoints', 'publicContracts', 'blastRadius'];
const RISK_GATE_KEYS = ['highRisk', 'autoStopRequired', 'humanApproved', 'largeDiff', 'reasons'];
const VERIFICATION_KEYS = ['commands', 'beforeAfter'];
const VERIFICATION_COMMAND_KEYS = ['command', 'status', 'exitCode', 'timestamp', 'summary'];
const REVIEW_DECISION_KEYS = ['decision', 'score', 'criticalCount', 'acceptanceCoverage', 'regressionProof', 'blockingReasons', 'contractStatus'];
const ADVERSARIAL_KEYS = ['decision', 'disprovenClaims', 'unverifiedClaims', 'missingProof', 'reachableRegressions'];

function validateContext(value) {
  const errors = [];
  if (!isObject(value)) return [{ path: 'context-snippets.json', message: 'must be an object' }];
  rejectUnknownKeys(errors, value, CONTEXT_KEYS);
...
function validateRiskGate(value) {
  ...
  rejectUnknownKeys(errors, value, RISK_GATE_KEYS);
...
function validateVerification(value) {
  ...
  rejectUnknownKeys(errors, value, VERIFICATION_KEYS);
  ...
  value.commands.forEach((command, index) => {
    ...
    for (const extra of Object.keys(command)) {
      if (!VERIFICATION_COMMAND_KEYS.includes(extra)) {
        push(errors, `${base}.${extra}`, 'unknown property (schema declares additionalProperties: false)');
      }
    }
...
function validateReviewDecision(value) {
  ...
  rejectUnknownKeys(errors, value, REVIEW_DECISION_KEYS);
...
function validateAdversarial(value) {
  ...
  rejectUnknownKeys(errors, value, ADVERSARIAL_KEYS);
  if (!DECISIONS.has(value.decision)) push(errors, 'decision', 'must be PASS, PASS_WITH_RISK, or BLOCKED');
