...
def _find_project_root() -> Optional[Path]:
    """Walk up from CWD to find project root (directory containing .git or .claude).

    Mirrors the logic in resolve_env.find_project_root() — kept inline here to
    avoid a circular import between common/ helpers.
...
def find_api_key(skill_dir: Optional[Path] = None) -> Optional[str]:
    ...
    if skill_dir is None:
        skill_dir = Path(__file__).parent  # .claude/skills/common/
    # Use CWD-based project root detection (matches resolve_env.find_project_root())
    # so that project_dir is always the actual project root regardless of call depth.
    project_dir = _find_project_root() or skill_dir.parent.parent.parent
