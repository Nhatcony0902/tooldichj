...
def load_env_files():
    ...
    2. .claude/skills/t1k-extended-multimodal/.env (skill-specific config)
    ...
    skill_dir = script_dir.parent  # .claude/skills/t1k-extended-multimodal
...
def _parse_rational(s: str) -> float:
    """Safely parse a rational string like '30/1' or '2997/100' into a float.

    Replaces the unsafe eval() that was previously used to compute FPS from
    ffprobe's r_frame_rate field.  A crafted media file could embed an
    arbitrary Python expression in that field; this function only accepts the
    expected NUM/DEN format.

    Args:
        s: Rational string from ffprobe (e.g. '30/1', '0/1', '2997/100')

    Returns:
        Float value, or 0.0 on any parse error.
    """
    try:
    ...
    try:
        ...
        for stream in data.get('streams', []):
            if stream['codec_type'] == 'video':
                ...
                info['fps'] = _parse_rational(stream.get('r_frame_rate', '0/1'))
