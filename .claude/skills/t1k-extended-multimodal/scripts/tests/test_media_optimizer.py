...
class TestParseRational:
    """Regression tests for _parse_rational (F-C01 fix — replaces eval())."""

    def test_normal_fps_30(self):
        """Standard 30fps rational."""
        assert mo._parse_rational('30/1') == pytest.approx(30.0)

    def test_normal_fps_ntsc(self):
        """NTSC 29.97fps rational."""
        assert mo._parse_rational('30000/1001') == pytest.approx(29.97, rel=1e-3)

    def test_normal_fps_24(self):
        """Cinema 24fps rational."""
        assert mo._parse_rational('24/1') == pytest.approx(24.0)

    def test_zero_default(self):
        """Default '0/1' returns 0.0."""
        assert mo._parse_rational('0/1') == 0.0

    def test_zero_denominator(self):
        """Division by zero returns 0.0, does not raise."""
        assert mo._parse_rational('30/0') == 0.0

    def test_no_denominator(self):
        """Plain integer string without slash."""
        assert mo._parse_rational('25') == pytest.approx(25.0)

    def test_adversarial_import_exec(self):
        """F-C01 regression: crafted r_frame_rate that would execute code via eval().

        The old code used eval() on this value; this test confirms the safe
        parser raises no exception and returns 0.0 instead of executing code.
        """
        malicious = "__import__('os').system('echo pwned')"
        result = mo._parse_rational(malicious)
        # Must not raise; must return the safe default
        assert result == 0.0

    def test_adversarial_expression(self):
        """Another eval-injection variant."""
        assert mo._parse_rational('1+1') == 0.0

    def test_adversarial_empty(self):
        """Empty string does not raise."""
        assert mo._parse_rational('') == 0.0
