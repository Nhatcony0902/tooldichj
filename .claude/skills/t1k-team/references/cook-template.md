...
1. **Re-verify Pre-flight Step 1 (Agent callable?).** Defense-in-depth for #259. If FAIL, bail per `fork-context-bail.md` — do NOT call `TeamCreate`/`TaskCreate`.
2. **Verify `TeamCreate` available.** `ToolSearch(query="select:TeamCreate", max_results=1)`. If unavailable, STOP per SKILL.md Step 2.
3. `TeamCreate(team_name: "<feature-slug>")`
4. `TaskCreate` x N dev tasks:
...
5. `TaskCreate` x 1 t1k-tester task:
...
**DO NOT** instruct the implementer to fan out further into sub-agents (SA-A / SA-B / SA-C partitioning). At fork depth 2 only the `Explore` agent type is reachable — `dots-implementer` / `general-purpose` / registry-routed types are NOT in scope inside a teammate context. See SKILL.md → Gotchas + https://github.com/The1Studio/theonekit-core/issues/266. If a teammate runs out of token budget, it should report a handoff request to team-lead; team-lead spawns a follow-up teammate at depth 1.
